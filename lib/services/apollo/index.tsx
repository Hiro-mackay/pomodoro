...
import { createApolloClient } from "@/services/aplloClient";
...
async function useGetHeaders(ctx) {
  if (typeof window !== "undefined") return null;
  if (typeof ctx.req === "undefined") return null;
  const auth = null
  if (!auth && !auth?.userCredential.token) return null;
  return {
    authorization: `Bearer ${auth.userCredential.token}`,
  };
}
